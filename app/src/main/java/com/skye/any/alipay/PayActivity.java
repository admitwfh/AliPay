...
import com.skye.any.alipay.adapter.PayGoodAdapter;
...
public class PayActivity extends AppCompatActivity {
    ...
    ListView lv_pay_good;
    Button btn_pay;
    Button btn_cancel;

    List<Good> goodList;

    PayGoodAdapter adapter;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_pay);
        lv_pay_good = (ListView) findViewById(R.id.lv_pay_good);
        btn_pay = (Button) findViewById(R.id.btn_pay);
        btn_cancel = (Button) findViewById(R.id.btn_cancel);

        DaoMaster.DevOpenHelper helper = new DaoMaster.DevOpenHelper(PayActivity.this, "Good.db");
        SQLiteDatabase database = helper.getWritableDatabase();
        DaoMaster daoMaster = new DaoMaster(database);
        GoodDao dao = daoMaster.newSession().getGoodDao();

        goodList = dao.queryBuilder().build().list();
        adapter = new PayGoodAdapter(this,goodList);
        lv_pay_good.setAdapter(adapter);
        //
    ...
    public void pay(View v) {
        ...
        for(Good good:goodList){
            buffName.append(good.getName()).append(",");
        }

        String orderInfo = getOrderInfo(buffName.toString(), "该测试商品的详细描述", "0.01");
    ...
    public void h5Pay(View v) {
        ...
        /**
         * url是测试的网站，在app内部打开页面是基于webview打开的，demo中的webview是H5PayActivity，
         * demo中拦截url进行支付的逻辑是在H5PayActivity中shouldOverrideUrlLoading方法实现，
         * 商户可以根据自己的需求来实现
         */
